...
namespace FinalProject.Front.Services
{
	public class AccountService
	{
		...
		public async Task<LoginResponseDto> LoginAsync(string email, string password)
		{
			...
			try
			{
				...
				if (response.IsSuccessStatusCode)
				{
					...
					ContextHelper.Token = result.Token;
					_httpClient.DefaultRequestHeaders.Authorization =
					new AuthenticationHeaderValue("Bearer", ContextHelper.Token);
					return result;
				}
		...
		//get markers
		public async Task<IEnumerable<UserProfileDto>> GetMarkersAsync()
		{
			var response = await _httpClient.GetAsync($"{_apiBaseUrl}/GetMarkers");
			response.EnsureSuccessStatusCode();
			using var responseContent = await response.Content.ReadAsStreamAsync();
			return await JsonSerializer.DeserializeAsync<IEnumerable<UserProfileDto>>(responseContent,
								new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
		}
	}
}
