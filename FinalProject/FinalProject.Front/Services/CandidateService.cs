...
namespace FinalProject.Front.Services
{
	...
	public class CandidateService
	{
		...
		//search candidate by number
		public async Task<CandidateDto> GetCandidateByNumberAsync(Guid candidateNumber)
		{
			var response = await _httpClient.GetAsync($"{_apiBaseUrl}/Number/{candidateNumber}");
			response.EnsureSuccessStatusCode();
			using var responseContent = await response.Content.ReadAsStreamAsync();
			var result = await JsonSerializer.DeserializeAsync<CandidateDto>(responseContent,
															 new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
